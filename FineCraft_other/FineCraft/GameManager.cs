...
namespace FineCraft
{
    public class GameManager
    {
        ...
        public GameDataProvider DataProvider { get { return dataProvider; } }
        ...
        {
            settings.Tick();
            renderer.SetDayTime(settings.dayTime);
            if (!Static.Paused)
            {
                Static.Volume.UpdatePosition();
                dataProvider.UpdateUserPosition(Static.UserData.Orientation);
            }
            dataProvider.UpdateDynamicObject(me);
        }

        public void DisplayLine(PChatLine line)
        {
            renderer.AddChatHistory(line.ToString());
        }
        ...
        private void KeyPress(object sender, KeyPressEventArgs er)
        {
            if (chat)
            {
                char c = er.KeyChar;
                if (c < 0x20) return;
                renderer.layerCurrentChatLine.Write(c);
                chatLine += c.ToString();
            }
        }
        private void KeyDown(object sender, KeyEventArgs er)
        {
            if (!chat)
                ...
                {
                    case Keys.Enter:
                        renderer.layerCurrentChatLine.Reset();
                        chatLine = "";
                        chat = true;
                        break;
                    ...
                    case Keys.G: renderer.FullScreen = !renderer.FullScreen; break;
                    case Keys.C:
                        var hit = RayTracer.FirstHit(this, rayTraceRange, Orientation.Position, Orientation.GetView());
                        if (hit != null && BlockTypes.FromID(GetValue(hit.X, hit.Y, hit.Z)).Removable)
                            SetValue(hit, 1);
                        break;
                    case Keys.V:
                        Vector3 side;
                    ...
                    case Keys.F2:
                        ...
                        try
                        {
                            renderer.ScreenShot.Save(dir + i + ".png", System.Drawing.Imaging.ImageFormat.Png);
                        }
                        catch { }
                        break;
                }
                ...
                switch (er.KeyCode)
                {
                    case Keys.Back:
                        if (chatLine.Length != 0)
                        {
                            ...
                            renderer.layerCurrentChatLine.Pop();
                        }
                        break;
                    case Keys.Enter:
                        if (chatLine.Trim() != "")
                            dataProvider.SendChatMessage(chatLine);
                        chat = false;
                        break;
                    case Keys.Escape:
                        chat = false;
        ...
        public void SetValue(WorldPosition pos, uint value)
        {
            ...
            dataProvider.UpdateChunk(new PChunkUpdate
            {
                Position = pos,
                NewId = value
            });
