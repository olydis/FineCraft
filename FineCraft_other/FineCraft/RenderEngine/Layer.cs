...
namespace FineCraft.RenderEngine
{
    ...
    public class Layer
    {
        ...
        public void Render()
        {
            ...
            renderer.GraphicsEffect.VPMatrix = proj;
            renderer.GraphicsEffect.WMatrix = Matrix.CreateTranslation(
                trans.X + (xDock == LayerDock.Center ? 0 : (xDock == LayerDock.Near ? -renderer.HalfWidth : renderer.HalfWidth)),
                trans.Y + (yDock == LayerDock.Center ? 0 : (yDock == LayerDock.Near ? -renderer.HalfHeight : renderer.HalfHeight)), 0);
            renderer.GraphicsEffect.Texture= textureMap;

            renderer.GraphicsEffect.BeginPass2D();
            renderer.GraphicsDevice.DrawUserPrimitives<VertexAwesome>(PrimitiveType.TriangleList, vertexBuffer, 0, 2 * index);
            renderer.GraphicsEffect.EndPass2D();
        }
    }
}
