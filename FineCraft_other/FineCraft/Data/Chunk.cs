...
namespace FineCraft.Data
{
    public class Chunk : AwesomeSerializable
    {
        ...
        public VertexBuffer[] ToDraw(GraphicsDevice dev)
        {
            ...
            for (uint x = 0; x < 16; x++)
                for (uint y = 0; y < 16; y++)
                    ...
                    doit:
                        if (block.IsCube)
                        {
                            bool solid = block.IsSolid; //false; //TODO - block.IsSolid;
                            int index = solid ? 0 : 6;
                        ...
                        if (block.Mesh != null)
                            buffer[6].AddRange(block.Mesh.Select(va =>
                            {
                                ...
                                va.TextureCoordinate.X = uv.X + GraphicsHelper.TextureXUnit.X * va.TextureCoordinate.X;
                                va.TextureCoordinate.Y = uv.Y + GraphicsHelper.TextureYUnit.Y * va.TextureCoordinate.Y;
                                return va;
                            }));
                    }
            bool nothing = true;
            var ret = buffer.Select(va =>
                {
                    if (va.Count == 0)
                        return null;
                    nothing = false;
                    VertexBuffer vb = new VertexBuffer(dev, VertexAwesome.SizeInBytes * va.Count, BufferUsage.WriteOnly);
                    vb.SetData<VertexAwesome>(va.ToArray());
                    return vb;
                }).ToArray();
            if (nothing)
                return null;
