...
namespace FineCraft.RenderEngine
{
    public class Renderer
    {
        ...
        void AdjustDevice()
        {
            if (Static.RenderTarget.Width > 0 && Static.RenderTarget.Height > 0)
                lock (pp)
                {
                    ...
                    SetFog((int)DynamicRangeChunkVolume.SIZE * 7);
                    ...
                    rtDepth = new RenderTarget2D(Static.Device, Static.Device.Viewport.Width, Static.Device.Viewport.Height, false, SurfaceFormat.Color, DepthFormat.Depth24Stencil8);
                    rtHigh = new RenderTarget2D(Static.Device, Static.Device.Viewport.Width, Static.Device.Viewport.Height, false, SurfaceFormat.Color, DepthFormat.Depth24Stencil8);
                    rtLow = new RenderTarget2D(Static.Device, Static.Device.Viewport.Width / 4, Static.Device.Viewport.Height / 4, false, SurfaceFormat.Color, DepthFormat.Depth24Stencil8);
        ...
        void Repaint()
        {
            ...
            lock (pp)
            {
                ...
                Action<RenderTarget2D, Color> renderWorld = (rt, back) =>
                {
                    ...
                    Static.Device.BlendState = BlendState.AlphaBlend;
                ...
                //try
                {
                    ...
                    renderWorld(rtHigh, Static.Effect.FogColor);

                    //Static.Effect.BeginPass3D();
                    //renderWorld(rtLow, Static.Effect.FogColor);

                    Static.Effect.BeginPassDepth();
                    if(Static.GameManager.HighDefinition)
                        renderWorld(rtDepth, Color.Black);
                    else
                    {
                        Static.Device.SetRenderTarget(rtDepth);
                    ...
                    Static.Effect.SetTexture("TDepth", rtDepth);
                    Static.Device.DrawUserPrimitives<VertexAwesome>(PrimitiveType.TriangleStrip, new VertexAwesome[]
                    {
                        new VertexAwesome(0, new Vector3(-100,-100,10), Vector3.Zero, Vector2.Zero),
                        new VertexAwesome(0, new Vector3(100,-100,10), Vector3.Zero, Vector2.UnitX),
                        new VertexAwesome(0, new Vector3(-100,100,10), Vector3.Zero, Vector2.UnitY),
                        new VertexAwesome(0, new Vector3(100,100,10), Vector3.Zero, Vector2.One), 
                    }, 0, 2);
        ...
        {
            lastDaytime = 17000; //daytime; //- todo
        }
    }
}
