...
namespace FineCraft.Data
{
    ...
    {
        const int SHIFT = 6;
        public const uint SIZE = 1 << SHIFT;
        ...
        public DynamicRangeChunkVolume(GraphicsDevice device, GameDataProvider provider)
        {
            ...
            volume = new Chunk[SIZE, SIZE, SIZE];
        ...
        public void UpdatePosition()
        {
            ...
            Chunk old;
            for (uint zz = unchecked((uint)(-SIZE / 4)+1); zz != SIZE / 4; zz++)
                for (uint xx = unchecked((uint)(-SIZE / 4)+1); xx != SIZE / 4; xx++)
                    for (uint yy = unchecked((uint)(-SIZE / 4)+1); yy != SIZE / 4; yy++)
                    {
        ...
        public List<WorldPosition> AligningEmissives(uint x, uint y, uint z)
        {
            List<WorldPosition> ret = new List<WorldPosition>();
            for (int zz = -1; zz <= 1; zz++)
                for (int yy = -1; yy <= 1; yy++)
                    for (int xx = -1; xx <= 1; xx++)
                    {
                        var temp = volume[coordToIndex((uint)(x + 16 * xx)), coordToIndex((uint)(y + 16 * yy)), coordToIndex((uint)(z + 16 * zz))];
                        if(temp != null)
                            ret.AddRange(temp.EmittingBlocks);
                    }
            return ret;
        }
        ...
        void RedrawWithSurrounding(uint chunkX, uint chunkY, uint chunkZ)
        {
            ...
            Redraw(chunkX + 1, chunkY, chunkZ);
            //shit
            Redraw(chunkX - 1, chunkY - 1, chunkZ - 1);
            Redraw(chunkX + 1, chunkY - 1, chunkZ - 1);
            Redraw(chunkX - 1, chunkY + 1, chunkZ - 1);
            Redraw(chunkX + 1, chunkY + 1, chunkZ - 1);
            Redraw(chunkX - 1, chunkY - 1, chunkZ + 1);
            ...
            Redraw(chunkX - 1, chunkY, chunkZ - 1);
            Redraw(chunkX + 1, chunkY, chunkZ - 1);
            Redraw(chunkX - 1, chunkY, chunkZ + 1);
            Redraw(chunkX + 1, chunkY, chunkZ + 1);

            Redraw(chunkX, chunkY - 1, chunkZ - 1);
            Redraw(chunkX, chunkY + 1, chunkZ - 1);
            Redraw(chunkX, chunkY - 1, chunkZ + 1);
            Redraw(chunkX, chunkY + 1, chunkZ + 1);

            Redraw(chunkX - 1, chunkY - 1, chunkZ);
            Redraw(chunkX + 1, chunkY - 1, chunkZ);
            Redraw(chunkX - 1, chunkY + 1, chunkZ);
            Redraw(chunkX + 1, chunkY + 1, chunkZ);
        }
        ...
        public void Render(BoundingFrustum frustum, WorldPosition hoverBox)
        {
            ...
            if (hoverBox != null)
            {
                Static.Device.BlendState = BlendState.Additive;
                Static.Effect.LightDirection = Vector3.Backward;
                Static.Effect.AmbientLightColor = Color.ForestGreen.ToVector4();
